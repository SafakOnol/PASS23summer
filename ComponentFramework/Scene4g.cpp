...
void Scene4g::HandleEvents(const SDL_Event &sdlEvent) {
	...
	//viewMatrix = sphereModelMatrix * sceneTrackball->getMatrix4();
...
void Scene4g::Render() const {
	...
	glUniformMatrix4fv(sphereShader->GetUniformID("viewMatrix"), 1, GL_FALSE, cameraSkybox->GetViewMatrix());
	//glUniform3fv(sphereShader->GetUniformID("lightPos"), 1, lightPos);

	// Sphere Render Data
	glBindTexture(GL_TEXTURE_2D, cameraSkybox->GetSkybox()->GetSkyboxTextureID());
	glUniformMatrix4fv(sphereShader->GetUniformID("modelMatrix"), 1, GL_FALSE, sphereModelMatrix);
	glBindTexture(GL_TEXTURE_CUBE_MAP, cameraSkybox->GetSkybox()->GetSkyboxTextureID());
